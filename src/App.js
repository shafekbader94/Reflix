...
import MovieDeatlis from "./components/MovieDeatlis";
...
class App extends Component {
  ...
  rentMovie = (userID, movieID) => {
    ...
    if (user.rented[movieID]) {
      user.budget = user.budget + 3;
      user.rented[movieID] = false;
    } else if (user.budget >= 3) {
      user.budget = user.budget - 3;
      user.rented[movieID] = true;
    } else alert("no enough $");
    users[userID] = user;
    this.setState({ users });
  };
      ...
      <Router>
        <div className="App">
          ...
          <div id="main-links">
            ...
            <span className="reflix">REFLIX</span>
          </div>
          ...
          <Route
            exact
            path="/"
            render={() => <Landing users={state.users} />}
          />
          <Route
            exact
            path="/catalog/:userID"
            render={(match) => (
              <Catalog
                user={state.users[match.match.params.userID]}
          ...
          <Route
            ...
            exact
            render={({ match }) => (
              <MovieDeatlis movie={state.movies[match.params.movieID]} />
            )}
          />
        </div>
      </Router>
